...
def sogou_weixin_search(query: Annotated[str, "搜索关键词"]) -> List[Dict[str, str]]:
    ...
    try:
        response = requests.get(
            "https://weixin.sogou.com/weixin", params=params, headers=headers
        )
...
def get_real_url(
    sogou_url: Annotated[str, "搜狗微信链接,来自于sogou_weixin_search工具结果"],
) -> str:
    """从搜狗微信链接获取真实的微信公众号文章链接"""
...
def get_article_content(
    real_url: Annotated[str, "真实微信公众号文章链接"],
    referer: Annotated[Optional[str], "请求来源,get_real_url的返回值"],
) -> str:
    """获取微信公众号文章的正文内容"""
    headers = {
        "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
...
def get_wechat_article(query: str, number=10):
    ...
    end_time = time.time()
    print(
        f"关键词{query}相关的文章已经获取完毕，获取到{len(articles)}篇, 耗时{end_time - start_time}秒"
    )
    return articles


mcp = FastMCP("Wechat Message Scraping")
...
def get_weixin_article_content(
    real_url: Annotated[str, "真实微信公众号文章链接"],
    referer: Annotated[Optional[str], "请求来源,weixin_search的返回值"],
) -> str:
    """获取微信公众号文章的正文内容
    Args:
