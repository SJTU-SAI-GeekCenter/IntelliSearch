...
from .types import TicketInfo, InterlineInfo
...
def extract_prices(yp_info: str, seat_discount_info: str,
                   ticket_data: Any) -> List[Dict[str, Any]]:
    """
...
def filter_tickets_info(
    ...
    if train_filter_flags:
        result = [
            ticket for ticket in tickets_info
    ...
    result = [
        ...
        if earliest_start_time <= int(ticket.start_time.split(':')[0]) < latest_start_time
    ]
...
def _match_train_filter(ticket: Any, flag: str) -> bool:
    ...
    code = ticket.start_train_code if hasattr(ticket, 'start_train_code') else ticket.start_train_code
    ...
    flag_map = {
        ...
        'O': lambda: not any(code.startswith(x) for x in ['G', 'C', 'D', 'Z', 'T', 'K']),
        'F': lambda: '复兴号' in ticket.dw_flag if hasattr(ticket, 'dw_flag') else False,
        'S': lambda: '智能动车组' in ticket.dw_flag if hasattr(ticket, 'dw_flag') else False,
    }
